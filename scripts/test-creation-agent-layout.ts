...
assert.match(shell, /agentOnly/);
assert.doesNotMatch(shell, /availableModes=/);
...
assert.match(workspace, /imageRefs: selectedReferences[\s\S]+reference\.status === 'ready'[\s\S]+reference\.kind === 'image'[\s\S]+Boolean\(reference\.url\)[\s\S]+\.map\(reference => reference\.url!\)/);
...
assert.doesNotMatch(workspace, /<Sparkles className="h-4 w-4" \/> Agent 模式/);
assert.match(workspace, /aria-label="画面规格"/);
assert.match(workspace, /aria-label="选择成片类型"/);
assert.doesNotMatch(workspace, />创作 Skill</);
