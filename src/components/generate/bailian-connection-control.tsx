...
export function BailianConnectionControl({
  ...
  useEffect(() => {
    ...
    setConfigured(planning.configured);
    setVideoConfigured(video.configured);
    setApiKey('');
    setError('');
    setNotice('');
  }, [storageScope]);
  ...
  const save = async () => {
    ...
    setConfigured(true);
    setVideoConfigured(result.videoReady);
    setApiKey('');
    setNotice(result.warning || '百炼规划、参考图与视频连接均已验证。');
    setOpen(!result.videoReady);
    onConnectionChange();
  };
  ...
  return (
    <div className="relative" data-testid="bailian-model-settings">
      ...
      >
        ...
        {configured ? (videoConfigured ? '百炼已连接' : '规划已连接') : '模型设置'}
      </button>
      ...
      {open ? (
        <div className="absolute right-0 top-full z-40 mt-2 w-[390px] max-w-[calc(100vw-32px)] rounded-2xl border border-[#e1e5eb] bg-white p-4 text-[#252931] shadow-[0_20px_50px_rgba(31,41,55,0.16)]">
          ...
          <label className="block text-xs font-medium text-[#626a76]">
            API Key
            <input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder={configured ? '重新填写后可更新连接' : '输入百炼 API Key'} autoComplete="new-password" className="mt-1.5 w-full rounded-lg border border-[#dfe4eb] bg-[#f9fafb] px-3 py-2 text-xs outline-none focus:border-[#9bb8ff] focus:bg-white" />
          </label>
          <div className="mt-3 grid grid-cols-1 gap-2 rounded-xl border border-[#e6eaf0] bg-[#f8f9fb] p-3 text-xs text-[#626a76] sm:grid-cols-3">
            ...
            <div><span className="block text-[10px] text-[#9299a4]">视频</span><span className="mt-1 block font-medium text-[#303640]">{DEFAULT_VIDEO_MODEL}</span>{configured && !videoConfigured ? <span className="mt-1 block text-[10px] text-[#b7791f]">需视频工作空间权限</span> : null}</div>
          </div>
          ...
          {notice ? <p className="mt-2 text-xs text-[#8a6418]">{notice}</p> : null}
