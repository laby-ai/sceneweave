...
export function VideoPromptGenerator({ onPromptGenerated, subtitleSegments = [] }: VideoPromptGeneratorProps) {
  ...
  const [autoGeneratedShots, setAutoGeneratedShots] = useState<PromptBasedShot[]>([]);
  // ★ v3.0 新增：叙事摘要和视觉锚点
  ...
  const generateShotsFromSubtitles = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.shots && data.shots.length > 0) {
        ...
        // ★ 自动同步总时长为所有镜头时长之和
        const calculatedTotal = data.shots.reduce((sum, shot) => sum + (shot.duration || 0), 0);
  ...
  // 使用运镜
  const applyCameraMovement = (movement: CameraMovement) => {
  ...
  // 使用快捷提示词
  const applyQuickPrompt = (type: 'basic' | 'advanced') => {
