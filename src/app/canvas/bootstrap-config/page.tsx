...
// 一键把画布 Agent config 指向本地 agent-proxy（火山 ARK），不依赖 icanvas 的配置 Modal。
...
export default function BootstrapConfigPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const baseChannel = { id: 'default', name: '默认渠道', baseUrl: PROXY_BASE, apiKey: PLACEHOLDER_KEY, apiFormat: 'openai' as const, models: [MODEL] };
      const updated = {
        ...parsed,
        version,
        state: {
          ...
          config: {
            ...
            textModel: MODEL,
            apiFormat: 'openai',
            channels: channels.length ? channels.map((ch: { name?: string; models?: string[] }) => ({ ...ch, baseUrl: PROXY_BASE, apiKey: PLACEHOLDER_KEY, apiFormat: 'openai', models: Array.isArray(ch.models) && ch.models.length ? ch.models : [MODEL] })) : [baseChannel],
            textModels: [MODEL],
            models: [MODEL],
          },
        },
