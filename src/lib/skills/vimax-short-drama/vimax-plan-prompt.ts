...
export function buildVimaxPlanMessages(prompt: string, preset: VimaxSkillPreset) {
  const system = [
    ...
    '剧情短剧默认用可见动作和人物对白推进。dialogue 只能写角色真正说出口的台词，不能写机位、画面说明或导演指令。',
    'narration 默认留空。只有 brief 明确要求旁白/解说，或必须压缩无法用画面和对白表达的时间、空间、背景事实时才填写；不得复述画面中已经发生的动作。',
    '严禁把“初见之时”“进一步了解”“最终我们看到的是”等通用介绍语，或“镜头展示/画面呈现/人物进入画面”等制作说明写成对白或旁白。',
