...
import { modelOptionName, normalizeModelOptionValue, resolveModelChannel, selectableModelsByCapability, useConfigStore, useEffectiveConfig, type AiConfig } from "@/icanvas/stores/use-config-store";
import { canvasThemes } from "@/icanvas/lib/canvas-theme";
import { nanoid } from "nanoid";
import { requestToolResponse, type ResponseFunctionTool, type ResponseInputMessage, type ResponseToolCall } from "@/icanvas/services/api/image";
import { imageToDataUrl } from "@/icanvas/services/image-storage";
import { useAssetStore } from "@/icanvas/stores/use-asset-store";
import { useThemeStore } from "@/icanvas/stores/use-theme-store";
import { useUserStore } from "@/icanvas/stores/use-user-store";
import { imageReferenceLabel } from "@/icanvas/lib/image-reference-prompt";
import { DiaTextReveal } from "@/icanvas/components/ui/dia-text-reveal";
import { Select, SelectContent, SelectItem, SelectTrigger } from "@/icanvas/components/ui/select";
...
const ONLINE_AGENT_PROMPT =
    "你是 Infinite Canvas 网页内置在线画布助手。当前画布 JSON 会随用户消息提供。首轮必须调用工具：只读问题调用 canvas_get_state，需要改动画布时调用和本地 Agent 一致的 infinite-canvas 工具。需要生成内容时直接调用 canvas_generate_text、canvas_generate_image、canvas_generate_video、canvas_generate_audio 或 canvas_create_generation_flow；需要精确批量操作时调用 canvas_apply_ops。不要输出 JSON ops，不要编造执行结果。创建短剧规划时，必须一次性创建 6 个 Clip + 6 个参考图提示词 + 1 个 final-video-plan 最终合成规划，合计 13 个独立节点；不要创建总纲、标题、项目说明或摘要节点占用这 13 个名额。每个文本节点先写 12-30 个中文字的简短可读内容，避免长段落；文本节点内容写入 metadata.content，也可以用 content 或 text 字段。工具参数涉及已有节点时必须使用当前画布 JSON 中真实存在的 id；缺少必要 id 或用户意图不明确时直接说明需要用户明确选择或说明，不要猜测。工具返回结果后，再根据真实结果回答用户。";
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition("canvas_apply_ops", "批量操作当前网页画布。ops 支持 add_node、update_node、delete_node、delete_connections、connect_nodes、set_viewport、select_nodes、run_generation。add_node 文本内容必须放到 metadata.content，也可放 content/text，系统会写入文本节点内容。短剧规划不要合并节点：6 Clip + 6 参考图提示词 + 1 final-video-plan 最终合成规划 = 13 个独立节点；不要创建总纲、标题、项目说明或摘要节点占用名额。每个节点内容先控制在 12-30 个中文字，避免长段落导致工具调用超时。", { ops: { type: "array", items: CANVAS_OP_SCHEMA } }, ["ops"], false),
